...
// Full state name → abbreviation map for search normalization
...
function buildSearchQuery(raw: string): string {
  ...
  // If it matches a full state name, return abbreviation
  if (STATE_ABBR[lower]) return STATE_ABBR[lower];

  // "City, ST" or "City ST" — normalize comma/space
...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/top50")
      .then((r) => r.json())
      .then((data) => {
        setTopFlagged(Array.isArray(data) ? data : []);
        setLoading(false);
      })
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-16">
      ...
      <div className="mb-20">
        ...
        {loading ? (
          <div className="py-8 text-center text-zinc-600 text-sm">Loading rankings...</div>
        ) : topFlagged.length === 0 ? (
